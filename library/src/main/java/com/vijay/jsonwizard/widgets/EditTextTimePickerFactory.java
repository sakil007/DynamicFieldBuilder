 ...
 */
public class EditTextTimePickerFactory implements FormWidgetFactory {
    ...
    public List<View> getViewsFromJson(String stepName, Context context, JSONObject jsonObject, CommonListener listener) throws Exception {
      ...
      /*  Drawable img = context.getResources().getDrawable( R.drawable.check_symbol );
        ...
        if(emailObject != null) {
            ...
            if (!TextUtils.isEmpty(emailValue)) {
                if (Boolean.TRUE.toString().equalsIgnoreCase(emailValue)) {
                    editText.addValidator(new RegexpValidator(emailObject.getString("err"), Patterns.EMAIL_ADDRESS.toString()));
                }
            }
        }
